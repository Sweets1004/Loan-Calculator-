...
export default function LoanCalculator() {
  ...
  const projectedValue = 545000 * Math.pow(1 + growthRate / 100, month / 12);
  ...
  return (
    <div className="p-4 max-w-xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4">
        <label>
          Loan Amount
          <input type="number" className="input" value={loanAmount}
            onChange={(e) => setLoanAmount(parseFloat(e.target.value))} />
        </label>
        <label>
          Interest Rate (%)
          <input type="number" step="0.01" className="input" value={interestRate}
            onChange={(e) => setInterestRate(parseFloat(e.target.value))} />
        </label>
        <label>
          Loan Term (Years)
          <input type="number" className="input" value={loanTerm}
            onChange={(e) => setLoanTerm(parseFloat(e.target.value))} />
        </label>
        <label>
          Extra Repayment / Month
          <input type="number" className="input" value={extraRepayment}
            onChange={(e) => setExtraRepayment(parseFloat(e.target.value))} />
        </label>
        <label>
          Annual Growth Rate (%)
          <input type="number" step="0.1" className="input" value={growthRate}
